...
export function ProductCard({ product, delay = 0 }: { product: Product; delay?: number }) {
  ...
  return (
    <article className={`${CATEGORY_CLASS[product.category]} animate-fade-up group`} style={{ animationDelay: `${delay}s` }}>
      <div className="tone-cover card-lift relative aspect-[4/5] overflow-hidden rounded-3xl shadow-[0_30px_80px_-45px_rgba(25,27,25,0.65)] transition duration-300 group-hover:-translate-y-1">
      ...
      <Link href={`/product/${product.slug}`} onClick={() => haptic()} className="mt-4 flex items-start justify-between gap-4 rounded-2xl transition-transform duration-200 group-hover:translate-x-1">
        <div>
