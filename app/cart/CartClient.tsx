...
export function CartClient() {
  ...
  return (
    <>
      <div className="mt-10">
        {resolved.map((line) => (
          <div key={line.slug} className={`${CATEGORY_CLASS[line.product.category]} hairline-b grid gap-4 py-5 sm:grid-cols-[72px_1fr_auto] sm:items-center lg:grid-cols-[80px_1fr_auto_auto_auto]`}>
            ...
            <div className="flex w-fit items-center gap-1 rounded-full bg-paper-2 p-1">
              <button onClick={() => setQty(line.slug, line.qty - 1)} className="tap flex h-8 w-8 items-center justify-center rounded-full hover:bg-paper" aria-label={`Decrease ${line.product.title}`}>
                <Minus className="h-3.5 w-3.5" />
              </button>
              <span className="data-num w-7 text-center text-sm">{line.qty}</span>
              <button onClick={() => setQty(line.slug, line.qty + 1)} className="tap flex h-8 w-8 items-center justify-center rounded-full hover:bg-paper" aria-label={`Increase ${line.product.title}`}>
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
            <div className="data-num text-right text-sm lg:w-20">${line.product.priceUsd * line.qty}</div>
            <button onClick={() => remove(line.slug)} className="tap flex h-9 w-9 items-center justify-center rounded-full text-ink-2 hover:bg-paper-2" aria-label={`Remove ${line.product.title}`}>
              <X className="h-4 w-4" />
            </button>
          </div>
        ))}
      ...
      <div className="hairline-t mt-8 flex flex-col gap-6 pt-7 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link href="/success" className="pill-solid tap w-fit" aria-label="Continue to checkout preview">
          Checkout preview
        </Link>
      </div>
    </>
